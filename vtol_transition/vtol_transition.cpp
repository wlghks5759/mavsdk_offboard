...
int main(int argc, char** argv)
{
    ...
    std::cout << "Setting takeoff altitude to 15m.\n";
    const Action::Result set_alt_result = action.set_takeoff_altitude(15.0);
    if (set_alt_result != Action::Result::Success) {
        std::cerr << "Failed to set takeoff altitude: " << set_alt_result << '\n';
        return 1;
    }
    ...
    //픽스트윙으로 될려면 충분한 고도가 필요함 14미터 정도
